...
export const createTableTAIKHOAN = function () {
    ...
    const sql = "CREATE TABLE `taikhoan`(`TaiKhoanID` int AUTO_INCREMENT PRIMARY KEY NOT NULL, `Ten` varchar(255) NOT NULL,`MatKhau` varchar(255) NOT NULL,`NgaySinh` date,`DiaChi` varchar(255),`SDT` varchar(255) UNIQUE,`Email` varchar(255) UNIQUE,`GioiTinh` tinyint DEFAULT 0,`HinhAnh` varchar(255),`LaAdmin` tinyint DEFAULT 0,`TrangThai` tinyint DEFAULT 0, `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL, `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP);";
